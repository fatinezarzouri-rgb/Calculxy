...
def group_triplets(
    ...
    for pf in pf_values:
        ...
        pl_candidates = [
            p for i, p in enumerate(pl_values)
            if i not in used_pl
            and abs(p["y"] - y) <= tolerance
        ]

        em_candidates = [
            e for i, e in enumerate(em_values)
            if i not in used_em
            and abs(e["y"] - y) <= tolerance
        ]
        ...
        pl_idx = pl_values.index(pl_match)
        em_idx = em_values.index(em_match)
